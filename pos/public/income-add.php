...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach (($data['items']['description'] ?? []) as $i => $desc) {
        ...
        $rows[] = ['desc'=>$desc,'qty'=>$qty,'price'=>$price,'total'=>$qty*$price];
        $subtotal += $qty * $price;
    }
    ...
    if ($edit_id) {
        db()->prepare('UPDATE income SET customer_id=?,customer_name=?,date=?,due_date=?,
                       subtotal=?,tax_pct=?,tax_amount=?,discount=?,total=?,paid=?,status=?,notes=?
                       WHERE id=? AND tenant_id=?')
            ->execute([$cust_id,$cust_name,$data['date'],$data['due_date']??null,
                       $subtotal,$tax_pct,$tax_amt,$discount,$total,$paid,$status,$data['notes']??'',
                       $edit_id,$tid]);
        ...
        db()->prepare('INSERT INTO income (tenant_id,branch_id,invoice_no,customer_id,customer_name,
                       date,due_date,subtotal,tax_pct,tax_amount,discount,total,paid,status,notes,created_by)
                       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)')
            ->execute([$tid,$bid,$inv_no,$cust_id,$cust_name,$data['date'],$data['due_date']??null,
                       $subtotal,$tax_pct,$tax_amt,$discount,$total,$paid,$status,$data['notes']??'',uid()]);
        $inc_id = db()->lastInsertId();
    }
    $ins = db()->prepare('INSERT INTO income_items (income_id,description,qty,unit_price) VALUES (?,?,?,?)');
    foreach ($rows as $r) $ins->execute([$inc_id,$r['desc'],$r['qty'],$r['price']]);
    db()->commit();
...
<div class="card" style="padding:16px;border:2px solid var(--c-primary,#4f46e5)">
  ...
  <div id="step_found" style="display:none;margin-top:8px;padding:12px;background:var(--c-bg);border:1px solid var(--c-income,#16a34a);border-radius:var(--radius)">
    ...
    <div style="margin-top:10px;display:flex;gap:8px">
      <button type="button" class="btn btn-primary btn-sm" id="btn_add_to_invoice" onclick="confirmAddFound()">➕ Add to Invoice</button>
...
<div class="card">
  ...
  <div class="table-wrap" style="overflow-x:auto">
    <table id="items_table" style="width:100%;table-layout:fixed">
      ...
      <?php if ($items): foreach ($items as $it): ?>
        <tr>
          <td><input name="items[description][]" value="<?= h($it['description']) ?>" required style="width:100%;box-sizing:border-box"></td>
          <td><input name="items[qty][]" type="number" step="0.001" value="<?= h($it['qty']) ?>" style="width:100%;box-sizing:border-box" oninput="posCalcItems()"></td>
          <td><input name="items[unit_price][]" type="number" step="0.01" value="<?= h($it['unit_price']) ?>" style="width:100%;box-sizing:border-box" oninput="posCalcItems()"></td>
          ...
          <td style="text-align:center"><button type="button" class="btn btn-danger btn-sm" onclick="this.closest('tr').remove();posCalcItems()">✕</button></td>
        </tr>
      <?php endforeach; else: ?>
        <tr>
          <td><input name="items[description][]" placeholder="Item description" required style="width:100%;box-sizing:border-box"></td>
          <td><input name="items[qty][]" type="number" step="0.001" value="1" style="width:100%;box-sizing:border-box" oninput="posCalcItems()"></td>
          <td><input name="items[unit_price][]" type="number" step="0.01" value="0.00" style="width:100%;box-sizing:border-box" oninput="posCalcItems()"></td>
  ...
  <button type="button" class="btn btn-outline" style="margin-top:10px" id="btn_add_line" onclick="posAddItemRow()">+ Add Line</button>
</div>
...
<div style="display:grid;grid-template-columns:1fr minmax(0,300px);gap:20px;align-items:start">
  ...
  <div class="card" style="min-width:0;overflow:hidden">
    <table class="totals-table" style="width:100%;table-layout:fixed">
      ...
      <tr>
        <td><?= TAX_LABEL ?> %: <input type="number" step="0.01" id="tax_pct" name="tax_pct" value="<?= h($inv['tax_pct']??'0') ?>" style="width:52px" oninput="posCalcItems()"></td>
        <td style="text-align:right;white-space:nowrap"><?= $currency_sym ?> <span id="disp_tax"><?= number_format($inv['tax_amount']??0,2) ?></span></td>
      </tr>
      <tr>
        <td>Discount</td>
        <td style="text-align:right"><input type="number" step="0.01" id="discount" name="discount" value="<?= h($inv['discount']??'0') ?>" style="width:80px;text-align:right;box-sizing:border-box" oninput="posCalcItems()"></td>
      </tr>
...
function confirmAddFound() {
  ...
  addRowToTable(foundItem.name, qty, price, parseFloat(foundItem.quantity), foundItem.unit);
...
function saveNewItem() {
  ...
  fetch(APP_URL + '/api-save-item.php', {method:'POST', body:fd})
    ...
    .then(function(data){
      if (data.error) { alert(data.error); return; }
      addRowToTable(data.name, 1, data.unit_price, data.quantity, data.unit);
      resetSearch();
...
function addManualItem() {
  ...
  var price = parseFloat(document.getElementById('new_item_price').value) || 0;
  if (!name) { alert('Item name is required'); return; }
  addRowToTable(name, 1, price, null, '');
  resetSearch();
}
...
function posAddItemRow() {
  ...
  inp2.addEventListener('input', posCalcItems);
  ...
  inp3.addEventListener('input', posCalcItems);
...
function addRowToTable(name, qty, price, stockQty, unit) {
  var row = posAddItemRow();
...
window.addEventListener('load', function() {
  ...
  document.querySelectorAll('#items_body .btn-danger').forEach(function(btn){
    btn.addEventListener('click', function(){ this.closest('tr').remove(); posCalcItems(); });
