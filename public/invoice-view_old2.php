...
$pm_labels = ['cash'=>'💵 Cash','card'=>'💳 Card','bkash'=>'💚 bKash','nagad'=>'📲 Nagad',
              'upay'=>'📱 Upay','vangri'=>'🪙 Vangri','bank'=>'🏦 Bank Transfer','other'=>'💬 Other'];
$s->execute([$id, $tid]);
$inv = $s->fetch();
...
<div class="invoice-box">
  ...
  <div style="margin-top:20px;display:flex;gap:12px;flex-wrap:wrap;align-items:flex-start">
    ...
    <div style="padding:8px 16px;background:#f0fdf4;border:1px solid #86efac;border-radius:8px;font-size:13px;font-weight:600;color:#166534">
      Payment: <?= h($pm_labels[$inv['payment_method']] ?? ucfirst($inv['payment_method'])) ?>
    </div>
    <?php endif; ?>
    <?php if ($inv['notes'] && !str_starts_with(trim($inv['notes']),'POS Sale')): ?>
    <div style="flex:1;padding:12px 14px;background:var(--c-bg);border-radius:var(--radius);font-size:13px">
      <strong>Notes:</strong><br><?= nl2br(h($inv['notes'])) ?>
    </div>
    <?php elseif ($inv['notes']): ?>
    <div style="flex:1;padding:8px 14px;background:var(--c-bg);border-radius:var(--radius);font-size:12px;color:var(--c-muted)">
      <?= nl2br(h($inv['notes'])) ?>
    </div>
    <?php endif; ?>
  </div>
