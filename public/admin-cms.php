...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'change_password') {
        ...
        if (!password_verify($cur, $row['password'])) {
            ...
            log_activity('update','Security','Admin password changed','UserID:'.uid());
    ...
    if ($action === 'create_user') {
        ...
        if (!$name || !$email || strlen($pass) < 6) {
            ...
            try {
                ...
                log_activity('create','Users','User created via CMS: '.$name,'Role:'.$role);
    ...
    if ($action === 'toggle_user') {
        ...
        if ($uid2 !== uid()) { // can't disable yourself
            ...
            log_activity('update','Users','User status toggled','UserID:'.$uid2);
    ...
    if ($action === 'save_reorder') {
        ...
        log_activity('update','Inventory','Reorder levels updated','Count:'.$count.' Branch:'.$bid_filter);
...
if ($tab === 'overview') {
    $stats = db()->prepare("SELECT
        (SELECT COUNT(*) FROM users WHERE tenant_id=? AND is_active=1) users,
        (SELECT COUNT(*) FROM items WHERE tenant_id=? AND is_active=1) items,
        (SELECT COUNT(*) FROM income WHERE tenant_id=? AND status='unpaid') unpaid_inv,
        (SELECT COUNT(*) FROM items WHERE tenant_id=? AND reorder_level>0 AND quantity<=reorder_level AND is_active=1) low_stock
    ");
    $stats->execute([$tid, $tid, $tid, $tid]);
    $stats = $stats->fetch();
}

if ($tab === 'users') {
    $all_users = db()->prepare('SELECT u.*, b.name branch_name FROM users u LEFT JOIN branches b ON b.id=u.branch_id WHERE u.tenant_id=? ORDER BY u.role DESC, u.name');
    $all_users->execute([$tid]);
    $all_users = $all_users->fetchAll();
}

if ($tab === 'reorder') {
    $bid_filter = (int)($_GET['bid'] ?? $bid_active);
    $reorder_items = db()->prepare('SELECT * FROM items WHERE tenant_id=? AND branch_id=? AND is_active=1 ORDER BY category, name');
    $reorder_items->execute([$tid, $bid_filter]);
    $reorder_items = $reorder_items->fetchAll();
}
    ...
    <div class="table-wrap">
      <table>
        ...
        <?php foreach ($all_users as $u): ?>
          <tr>
            <td><strong><?= h($u['name']) ?></strong><?= $u['id']==uid() ? ' <span style="font-size:11px;color:var(--c-muted)">(you)</span>' : '' ?></td>
            <td style="font-size:13px"><?= h($u['email']) ?></td>
            <td><span class="badge badge-<?= $u['role']==='admin'||$u['role']==='superadmin'?'paid':'partial' ?>"><?= ucfirst($u['role']) ?></span></td>
...
<form method="post">
  ...
  <div class="card" style="padding:0">
    ...
    <div class="table-wrap">
      <table>
        <thead>
          <tr><th>Item</th><th>Category</th><th style="text-align:right">Current Stock</th><th style="text-align:right;width:160px">Reorder Level</th><th>Status</th></tr>
        </thead>
        <tbody>
        ...
        ?>
          <tr>
            ...
            <td style="text-align:right">
              <input type="number" name="reorder_level[]" value="<?= h($it['reorder_level'] + 0) ?>"
        ...
        <?php if (!$reorder_items): ?>
          <tr><td colspan="5" style="text-align:center;color:var(--c-muted);padding:24px">No items in this branch yet.</td></tr>
        <?php endif; ?>
        </tbody>
      </table>
    </div>
