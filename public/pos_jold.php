  ...
  <div class="pos-left" id="pos_left">
    ...
    <div class="pos-cart-table-wrap">
      <table class="pos-cart-table" id="cart_table">
        ...
        <tbody id="cart_tbody">
          <tr class="cart-empty-row"><td colspan="5">🛒 No items yet — scan barcode or click a product</td></tr>
        </tbody>
      </table>
    </div>
...
function renderProducts(products){
  ...
  document.getElementById('products_grid').innerHTML=shown.map(p=>{
    ...
    return `<div class="product-card ${oos?'out-of-stock':''}" id="pc_${p.id}" onclick="${oos?'':'addToCart('+p.id+')'}">
      ...
      <div class="p-price">${CUR} ${parseFloat(p.unit_price).toFixed(2)}</div>
...
function addToCartById(itemId){
  ...
  else{cart.push({id:p.id,name:p.name,price:parseFloat(p.unit_price),qty:1,stock:parseFloat(p.quantity),unit:p.unit,sku:p.sku||''});}
  renderCart();
}
...
function renderCart(){
  ...
  if(!cart.length){tbody.innerHTML='<tr class="cart-empty-row"><td colspan="5">🛒 No items yet — scan barcode or click a product</td></tr>';document.getElementById('btn_checkout').disabled=true;recalc();return;}
  tbody.innerHTML=cart.map((item,i)=>`<tr>
    ...
    <td class="td-total">${CUR} ${(item.price*item.qty).toFixed(2)}</td>
...
function recalc(){
  ...
  const total=Math.max(0,sub-discAmt+shipping),itemCount=cart.reduce((s,i)=>s+i.qty,0);
...
function checkout(){
  ...
  const sub=cart.reduce((s,i)=>s+i.price*i.qty,0),shipping=parseFloat(document.getElementById('pos_shipping').value)||0;
  let discAmt=discMode==='pct'?sub*(parseFloat(document.getElementById('pos_discount').value)||0)/100:parseFloat(document.getElementById('pos_discount').value)||0;
  const total=Math.max(0,sub-discAmt+shipping),custId=document.getElementById('pos_cust_id').value,custName=document.getElementById('pos_cust_input').value.trim()||'Walk-in Customer',today=new Date().toISOString().slice(0,10);
  ...
  document.getElementById('fc_notes').value='POS Sale · Payment: '+payMethod+(document.getElementById('pos_ref').value?' · Ref: '+document.getElementById('pos_ref').value:'');
  const fc=document.getElementById('fc_items');fc.innerHTML='';
  cart.forEach(item=>{fc.innerHTML+=`<input type="hidden" name="items[description][]" value="${esc(item.name)}">`;fc.innerHTML+=`<input type="hidden" name="items[qty][]" value="${item.qty}">`;fc.innerHTML+=`<input type="hidden" name="items[unit_price][]" value="${item.price}">`;fc.innerHTML+=`<input type="hidden" name="items[item_id][]" value="${item.id}">`;});
...
function printReceipt(){
  ...
  let discAmt=discMode==='pct'?sub*(parseFloat(document.getElementById('pos_discount').value)||0)/100:parseFloat(document.getElementById('pos_discount').value)||0;
  const total=Math.max(0,sub-discAmt+shipping);
  ...
  const rows=cart.map(item=>`
    <div class="th-item-row">
      ...
      <div class="th-item-qty">${item.qty.toFixed(item.qty%1===0?0:2)} x ${CUR}${item.price.toFixed(2)}</div>
      <div class="th-item-total">${CUR}${(item.price*item.qty).toFixed(2)}</div>
    </div>`).join('');
