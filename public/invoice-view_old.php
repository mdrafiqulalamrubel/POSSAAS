...
// Resolve logo URL (no file_exists — blocked by open_basedir on shared hosting)
$logo_url = '';
$logo_path = $cs['logo_path'] ?? '';
if ($logo_path) {
    $logo_url = APP_URL . '/' . ltrim($logo_path, '/');
}

ob_start();
?>
<div class="no-print" style="display:flex;gap:10px;margin-bottom:20px">
  <button onclick="printA4()" class="btn btn-outline no-print" style="margin-right:4px">🖨 A4 Print</button>
  <button type="button" onclick="printThermal()" class="btn btn-outline no-print" style="margin-right:4px">🧾 Thermal (80mm)</button>
...
.print-a4-wrap  { width:210mm; margin:0 auto; }
.print-80mm-wrap{ width:80mm;  margin:0 auto; font-size:11px; }
</style>
<script>
function printA4() {
